...
const MenuSection = () => {
  ...
  return (
    <section id="menu" className="py-20 bg-foreground text-background">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16" dir={isRTL ? "rtl" : "ltr"}>
          <span className="text-accent font-medium text-sm uppercase tracking-wider">{t("menu.tagline")}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" dir={isRTL ? "rtl" : "ltr"}>
          {menuItems.map((item, index) => (
            <Card
              key={index}
              className="bg-background/5 border-background/10 backdrop-blur-sm overflow-hidden group hover:bg-background/10 transition-colors"
            >
              ...
              <CardContent className="p-5">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-lg font-bold text-background">{t(item.nameKey)}</h3>
                  <span className="text-accent font-bold text-lg">{item.price} {t("menu.currency")}</span>
                </div>
                <p className="text-background/60 text-sm">{t(item.descKey)}</p>
              </CardContent>
            </Card>
          ))}
        </div>
